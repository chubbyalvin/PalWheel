...
function PalActions:getHolder()
    ...
    local pc = o.state.pc
    if not o.alive(pc) and type(o.getPlayerController) == "function" then
        pc = o.getPlayerController()
    end
    if o.alive(pc) then
        local holder = nil
        local okLocal = pcall(function() holder = pc.BP_OtomoPalHolderComponent end)
        if okLocal and o.alive(holder) then
...
function PalActions:getPartyCapacity(holder)
    local o = self.options
    if not o.alive(holder) then holder = self:getHolder() end
    if not o.alive(holder) then return nil end

    local total = nil
    local okTotal = pcall(function() total = holder:GetMaxOtomoNum() end)
    total = okTotal and tonumber(total) or nil
    if total == nil then return nil end
    total = math.floor(total)
    if total < 1 then return nil end
    if total > MAX_PARTY_CAPACITY then total = MAX_PARTY_CAPACITY end
    return total
end
...
function PalActions:readSelectedSlot(holder)
    ...
    if not o.alive(holder) then holder = self:getHolder() end
    if not o.alive(holder) then return nil end
    local capacity = self:getPartyCapacity(holder) or DEFAULT_PARTY_CAPACITY
    local selected = nil
    local okSelected = pcall(function() selected = holder:GetSelectedOtomoID() end)
    selected = okSelected and tonumber(selected) or nil
    if selected == nil or selected < 0 or selected >= capacity then return nil end
...
function PalActions:selectSlotNatively(pc, index, source)
    ...
    if index < 0 or index >= capacity then
        o.log("Party Pal slot " .. tostring(index + 1)
            .. " unavailable; detected party capacity is " .. tostring(capacity), true)
        return false
    end
    ...
    local forward = (index - current) % capacity
    local backward = (current - index) % capacity
    local direction = backward < forward and -1 or 1
    local steps = math.min(forward, backward)

    for _ = 1, math.max(1, steps) do
    ...
    o.log("Native party selection could not reach slot " .. tostring(index + 1)
        .. " within detected capacity " .. tostring(capacity), true)
    return false
end
...
function PalActions:activateCurrentNearPlayer(pc, index)
    local o = self.options
    local okSummon, summonErr = pcall(function()
        pc:ActivateCurrentOtomoNearThePlayer()
    end)
